...
| Column | Description | Required | Type | Example | Used for |
| --- | --- | --- | --- | --- | --- |
| `tournament_year` | Historical World Cup year. | Yes | number | 2018 | matching |
| `player_name` | Exact player name from the historical training file. | Yes | string | source player name | matching |
| `country` | Country context for QA. | Optional | string | France | audit |
| `position` | Position context for QA. | Optional | string | Forward | audit |
...
| `senior_national_team_caps` | Senior caps before tournament start. Do not approximate from World Cup appearances. | Optional | number | 35 | modelling |
| `injury_availability_score` | Real or documented availability score scaled 0-100. | Optional | number | 95.0 | modelling |
| `recent_form_score` | Real or documented form score scaled 0-100. | Optional | number | 82.0 | modelling |
| `role_fit_score` | Real or documented tactical role-fit score scaled 0-100. | Optional | number | 76.0 | modelling |
| `data_source` | Source name for supplied values. Required when any value is supplied. | Conditional | string | Licensed club export | audit |
| `source_url` | Source URL, local export path, or license note. | Recommended | string | data/local/export.csv | audit |
| `notes` | Matching, source, or caveat notes. | Optional | string | exact FIFA squad match | audit |
...
| `expected_impact_model_mode` | Active expected-impact model mode: limited historical context, full recruitment, or baseline fallback. | Optional | string | limited_historical_context_model | audit, display |
| `expected_impact_model_features_used` | Comma-separated active supervised features used by the trained model. | Optional | string | age, position, previous_world_cup_minutes | audit, methodology |
| `expected_impact_model_warning` | Warning or limitation text for the active model mode. | Optional | string | limited model warning | audit, display |
| `model_confidence_score` | Row-level completeness score for the active model features before sklearn pipeline imputation. | Optional | number | 55.6 | modelling, display |
