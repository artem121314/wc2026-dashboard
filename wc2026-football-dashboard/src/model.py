...
MIN_USABLE_PREDICTORS_FOR_MODEL = 3
...
LIMITED_MODEL_MESSAGE = (
    "Expected impact is generated by a limited supervised historical model trained on real World Cup-derived features. "
    "Rich recruitment predictors such as market value and club-season stats are not yet available for historical "
    "training, so this should be treated as an MVP model."
)
FULL_MODEL_MESSAGE = "Supervised expected-impact model is enabled as a full recruitment model trained on enriched real historical player-tournament data."
...
LIMITED_HISTORICAL_CONTEXT_FEATURES = [
    "age",
    "position",
    "is_world_cup_debutant",
    "previous_world_cup_minutes",
...
MODEL_MODE_FULL = "full_recruitment_model"
MODEL_MODE_LIMITED = "limited_historical_context_model"
MODEL_MODE_BASELINE = "baseline_fallback"
...
def historical_feature_availability(data: pd.DataFrame) -> pd.DataFrame:
    ...
    for feature in EXPECTED_IMPACT_FEATURES:
        ...
        elif feature == "tournament_year":
            feature_group = "tournament_context"
            requirement = "limited_optional"
        else:
            feature_group = "optional"
            requirement = "optional"
        required = requirement in {"limited_required"}
        ...
        minimum = MIN_REQUIRED_FEATURE_COVERAGE_PCT if requirement != "optional" else 1.0
        usable = bool(non_null > 0 and distinct > 1 and (requirement == "optional" or coverage >= minimum))
        if feature not in data.columns:
            excluded_reason = "missing column"
        elif non_null == 0:
            excluded_reason = "no real values"
        elif distinct < 2:
            excluded_reason = "no variation"
        elif coverage < minimum:
            excluded_reason = f"coverage below {minimum:.0f}%"
        else:
            excluded_reason = ""
        rows.append(
            {
...
def model_activation_report(data: pd.DataFrame) -> dict[str, object]:
    ...
    availability = historical_feature_availability(data)
    below_limited_required = availability[
        availability["requirement"].eq("limited_required") & ~availability["usable_for_modelling"]
    ...
    usable_limited = [
        feature
        for feature in LIMITED_HISTORICAL_CONTEXT_FEATURES
        if feature
        in set(availability.loc[availability["usable_for_modelling"], "feature"].astype(str))
    ]
    limited_ready = bool(
        not below_limited_required and len(usable_limited) >= MIN_USABLE_PREDICTORS_FOR_MODEL
    )
    ...
    team_optional_missing = [
        feature
        for feature in TEAM_TOURNAMENT_CONTEXT_FEATURES
        if feature not in set(usable_by_group.get("team_tournament_context", []))
    ]
    if not recruitment_ready:
        missing_groups.append("recruitment_pre_tournament")

    full_ready = bool(not below_limited_required and recruitment_ready)
    if full_ready:
        mode = MODEL_MODE_FULL
    elif limited_ready:
        mode = MODEL_MODE_LIMITED
    else:
        mode = MODEL_MODE_BASELINE
    activation_ready = mode in {MODEL_MODE_FULL, MODEL_MODE_LIMITED}
    return {
        "activation_ready": activation_ready,
        "predictor_coverage_sufficient": activation_ready,
        "full_recruitment_model_available": full_ready,
        "limited_historical_context_model_available": limited_ready,
        "selected_model_mode": mode,
        "required_features_below_minimum_coverage": below_limited_required,
        ...
        "optional_team_context_features_excluded": team_optional_missing,
        "usable_limited_features": usable_limited,
        "usable_limited_feature_count": len(usable_limited),
        "minimum_limited_features_required": MIN_USABLE_PREDICTORS_FOR_MODEL,
...
def check_historical_model_readiness(
    ...
    readiness: dict[str, object] = {
        ...
        "selected_model_mode": MODEL_MODE_BASELINE,
        "model_mode": MODEL_MODE_BASELINE,
        "full_recruitment_model_available": False,
        "limited_historical_context_model_available": False,
        "missing_predictor_groups": [],
        "optional_team_context_features_excluded": [],
        "usable_limited_features": [],
        "usable_limited_feature_count": 0,
        "minimum_limited_features_required": MIN_USABLE_PREDICTORS_FOR_MODEL,
    ...
    readiness["selected_model_mode"] = str(activation_report["selected_model_mode"])
    readiness["model_mode"] = str(activation_report["selected_model_mode"])
    readiness["full_recruitment_model_available"] = bool(activation_report["full_recruitment_model_available"])
    readiness["limited_historical_context_model_available"] = bool(
        activation_report["limited_historical_context_model_available"]
    )
    readiness["missing_predictor_groups"] = list(activation_report["missing_predictor_groups"])
    readiness["optional_team_context_features_excluded"] = list(
        activation_report["optional_team_context_features_excluded"]
    )
    readiness["usable_limited_features"] = list(activation_report["usable_limited_features"])
    readiness["usable_limited_feature_count"] = int(activation_report["usable_limited_feature_count"])
    ...
    if readiness["can_train"] and attempt_training:
        ...
        if readiness["model_available"]:
            ...
            readiness["readiness_message"] = str(model_summary.get("readiness_message", MODEL_ENABLED_MESSAGE))
            readiness["selected_model_mode"] = str(model_summary.get("model_mode", readiness["selected_model_mode"]))
            readiness["model_mode"] = readiness["selected_model_mode"]
        ...
        readiness["readiness_message"] = (
            LIMITED_MODEL_MESSAGE
            if readiness["selected_model_mode"] == MODEL_MODE_LIMITED
            else FULL_MODEL_MESSAGE
        )
    ...
    elif readiness["selected_model_mode"] == MODEL_MODE_BASELINE:
    ...
    for alias in [target, *aliases]:
        if alias in df.columns:
            ...
            if target in CATEGORICAL_FEATURES:
                ...
                values = values.where(values.notna(), candidate_numeric)
...
def select_expected_impact_features(training_data: pd.DataFrame) -> list[str]:
    ...
    activation = model_activation_report(training_data)
    if activation["selected_model_mode"] == MODEL_MODE_LIMITED:
        ordered_features = LIMITED_HISTORICAL_CONTEXT_FEATURES
...
def prepare_expected_impact_features(
    ...
    mapped["tournament_year"] = _coalesce_feature(df, "tournament_year", [], 2026)
...
def _excluded_features_table(training_data: pd.DataFrame, selected_features: list[str]) -> pd.DataFrame:
    availability = historical_feature_availability(training_data)
    excluded = availability[~availability["feature"].isin(selected_features)].copy()
    columns = [
        "feature",
        "feature_group",
        "requirement",
    ...
    return excluded[columns].reset_index(drop=True)
...
def _empty_training_result(model_type: str = "gradient_boosting") -> dict[str, object]:
    return {
        ...
        "model_mode": MODEL_MODE_BASELINE,
        "expected_impact_model_mode": MODEL_MODE_BASELINE,
        "expected_impact_model_features_used": "",
        "expected_impact_model_warning": FALLBACK_WARNING,
        "features_used": [],
        "features_excluded": pd.DataFrame(columns=["feature", "excluded_reason"]),
...
def train_expected_impact_model(
    ...
    if not activation_report["activation_ready"]:
        ...
        result["limited_historical_context_model_available"] = bool(
            activation_report["limited_historical_context_model_available"]
        )
        result["full_recruitment_model_available"] = bool(activation_report["full_recruitment_model_available"])
        return result

    feature_columns = select_expected_impact_features(training_data)
    model_mode_selected = str(activation_report["selected_model_mode"])
    if len(feature_columns) < MIN_USABLE_PREDICTORS_FOR_MODEL:
        result = _empty_training_result(model_type)
        result["model_status"] = "disabled_missing_real_pre_tournament_features"
        result["warning"] = (
            PREDICTOR_COVERAGE_WARNING
            + f" Fewer than {MIN_USABLE_PREDICTORS_FOR_MODEL} usable real predictors are available after coverage checks."
        )
        result["readiness_message"] = PREDICTOR_COVERAGE_WARNING
        result["training_row_count"] = int(len(training_data))
        return result
    ...
    return {
        ...
        "model_mode": model_mode_selected,
        "expected_impact_model_mode": model_mode_selected,
        "target_source": "historical World Cup actual_tournament_impact_score",
        "warning": LIMITED_MODEL_MESSAGE if model_mode_selected == MODEL_MODE_LIMITED else "",
        "training_row_count": int(len(training_data)),
        "data_source_path": str(HISTORICAL_TRAINING_PATH),
        "model_features": feature_columns,
        "features_used": feature_columns,
        "expected_impact_model_features_used": ", ".join(feature_columns),
        "expected_impact_model_warning": LIMITED_MODEL_MESSAGE if model_mode_selected == MODEL_MODE_LIMITED else "",
        "features_excluded": _excluded_features_table(training_data, feature_columns),
        "metrics": metrics_df,
        "feature_importance": get_feature_importance(selected_model, selected_name),
        "readiness_message": LIMITED_MODEL_MESSAGE if model_mode_selected == MODEL_MODE_LIMITED else FULL_MODEL_MESSAGE,
        "predictor_coverage_sufficient": True,
        "missing_predictor_groups": list(activation_report["missing_predictor_groups"]),
        "usable_recruitment_predictors": list(activation_report["usable_recruitment_predictors"]),
        "limited_historical_context_model_available": bool(activation_report["limited_historical_context_model_available"]),
        "full_recruitment_model_available": bool(activation_report["full_recruitment_model_available"]),
    }
...
def model_feature_completeness(df: pd.DataFrame, trained: dict[str, object]) -> pd.Series:
    """Score how much real current-player data exists for the active model features."""
    ...
    if not feature_columns:
        return pd.Series(pd.NA, index=df.index)
    X = prepare_expected_impact_features(df, feature_columns)
    completeness = pd.DataFrame(index=X.index)
    for feature in feature_columns:
        if feature in CATEGORICAL_FEATURES:
            values = X[feature].astype(str).str.strip()
            completeness[feature] = values.ne("").fillna(False) & values.str.lower().ne("unknown")
        else:
            completeness[feature] = pd.to_numeric(X[feature], errors="coerce").notna()
    return (completeness.mean(axis=1) * 100).clip(0, 100).round(1)
